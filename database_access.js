...
exports.loadTeams = function (connection, competition_id, players, teams, indForTid, callback){
    connection.query("SELECT * FROM teams WHERE competition_id = ?", competition_id, (err, rows, fields) => {
        ...
        for (let i=0; i<rows.length; ++i){
            ...
            teams.push(new objects.Team(rows[i].id, rows[i].name, rows[i].school, [], rows[i].points, rows[i].point_difference));
        }
...
exports.loadBattles = function (connection, competition_id, battles, indForBid, callback){
    connection.query("SELECT battles.id, battles.day, battles.team1, battles.team2, battles.points1, battles.points2, battles.judges, battles.challenges FROM battles INNER JOIN teams ON battles.team1 = teams.id WHERE teams.competition_id = ?", competition_id, (err, rows, fields) => {
        if (err){console.error(err);}
        for (let i=0; i<rows.length; ++i){
            indForBid[rows[i].id] = battles.length;
            battles.push(new objects.Battle(rows[i].id, rows[i].day, rows[i].team1, rows[i].team2, rows[i].points1, rows[i].points2, JSON.parse(rows[i].judges), JSON.parse(rows[i].challenges)));
        }
        if (callback) callback();
    });
}
